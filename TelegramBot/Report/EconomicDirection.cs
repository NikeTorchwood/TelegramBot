...
namespace TelegramBot.Report
{
    public class EconomicDirection
    {
        ...
        {
            Name = name;
            var dataSheet = ReportHelper.GetSheet("Данные ОП", workbook);
            ...
            Plan = dataSheet.Cells[row, planColumn].Type != CellValueType.IsString ? dataSheet.Cells[row, planColumn].IntValue : 0;
            Fact = dataSheet.Cells[row, factColumn].Type != CellValueType.IsString ? dataSheet.Cells[row, factColumn].IntValue : 0;
            var rankSheet = ReportHelper.GetSheet("Рейтинг ОП", workbook);
